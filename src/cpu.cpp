...
void CPU::setHL(uint16_t val) {
    ...
    _L = val & 0xFF;
    /*if (getHL() < 0x8000) {
        std::cout << "⚠️  HL set to ROM address: 0x" << std::hex << getHL() 
                  << " at PC: 0x" << _PC << "\n";
    }
    if (getHL() < 0x8000) {
        std::cerr << "❗ HL set to ROM area: 0x" << std::hex << getHL() << " from POP HL at PC: 0x" << _PC << "\n";
    }*/
}
...
void CPU::executeOpcode(uint8_t opcode) {
    ...
    switch (opcode) {
        ...
        case 0x31: // LD SP, d16
            ...
            break;
        case 0x32: // LD (HL-), A
            _mem.write(getHL(), _A);     // write A to address HL
            setHL(getHL() - 1);          // then decrement HL
            break;
        case 0x33: // INC SP
            setSP(getSP() + 1);
            break;
        ...
        case 0x39: // ADD HL, SP
            ...
            break;}
        case 0x3A: // LD A, (HL-)
            _A = _mem.read(getHL());     // read from HL
            setHL(getHL() - 1);          // then decrement HL
            break;
        case 0x3B: // DEC SP
            setSP(getSP() - 1);
        ...
        case 0x71: // LD (HL), C
            ...
            break;
        case 0x72: // LD (HL), D
            //std::cout << "LD (HL), D -- HL = 0x" << std::hex << getHL() << "\n";
            _mem.write(getHL(), _D);
            break;
        case 0x73: // LD (HL), E
            _mem.write(getHL(), _E);
